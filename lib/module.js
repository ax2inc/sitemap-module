...
module.exports = function module (moduleOptions) {
  let sitemaps = []
  if (Array.isArray(this.options.sitemap)) {
    sitemaps = this.options.sitemap
  } else {
    sitemaps.push(this.options.sitemap)
  }

  checkPathnamesValidity(sitemaps)

  for (let i = 0; i < sitemaps.length; i++) {
    const defaults = {
      path: sitemaps[i].path || 'sitemap.xml',
...
function checkPathnamesValidity (sitemaps) {
  if (sitemaps.filter(each => !each.path).length > 1) {
    throw new Error('Your sitemaps need different pathnames')
  }
  for (let i = 0; i < sitemaps.length; i++) {
    if (sitemaps.filter(each => each.path === sitemaps[i].path).length > 1) {
      throw new Error('Your sitemaps need different pathnames')
    }
  }
}
