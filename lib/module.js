...
module.exports = function module (moduleOptions) {
  const sitemaps = Array.isArray(this.options.sitemap) ? this.options.sitemap : [this.options.sitemap]
  ...
  // test configuration to make sure pathnames are unique
  if (sitemaps.length > 1) {
    checkPathnamesValidity(sitemaps, defaults.path)
  }

  for (let i = 0; i < sitemaps.length; i++) {
    const options = Object.assign({}, defaults, sitemaps[i], moduleOptions)
...
// Throw Errors if sitemaps paths have mistakes
function checkPathnamesValidity (sitemaps, defaultsPath = 'sitemap.xml') {
  // Make sure that multiple sitemaps won't have pathname 'sitemap.xml'
  if (sitemaps.filter(sitemap => !sitemap.path || sitemap.path === defaultsPath).length > 1) {
    throw new Error('Please specify different pathnames for your sitemaps')
  }
  // Make sure that multiple sitemaps don't have the same pathname
  for (let i = 0; i < sitemaps.length; i++) {
    if (sitemaps.find(sitemap => {
      return (sitemap.path === sitemaps[i].path) && sitemaps.indexOf(sitemap) !== i
    })) {
      throw new Error('Your sitemaps need unique pathnames')
    }
  }
}
