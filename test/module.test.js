...
describe('ssr multiple sitemaps', () => {
  ...
  test('sitemaps', async () => {
    ...
    expect(xml).toContain('<loc>http://localhost:3000/1</loc>')
    expect(xml).toContain('<loc>http://localhost:3000/child/1</loc>')
    expect(xml).not.toContain('<loc>http://localhost:3000/2</loc>')
    expect(xml).not.toContain('<loc>http://localhost:3000/child/2</loc>')

    const secondXml = await get('/second-sitemap.xml')
    expect(secondXml).toContain('<loc>http://localhost:3000/2</loc>')
    expect(secondXml).toContain('<loc>http://localhost:3000/child/2</loc>')
    expect(secondXml).not.toContain('<loc>http://localhost:3000/1</loc>')
    expect(secondXml).not.toContain('<loc>http://localhost:3000/child/1</loc>')
  })

  test('sitemaps gzip', async () => {
    const xml = await get('/sitemap.xml')
    const gz = await getGzip('/sitemap.xml.gz')
    const sitemap = gunzipSync(gz).toString()
    expect(xml).toEqual(sitemap)
    ...
    const secondXml = await get('/second-sitemap.xml')
    const secondGz = await getGzip('/second-sitemap.xml.gz')
    const secondSitemap = gunzipSync(secondGz).toString()
    expect(secondXml).toEqual(secondSitemap)
  })
})
